<x-app-layout>
    <x-slot name="header">
        {{ isset($jenis) && $jenis == 'ditemukan' ? 'Laporkan Barang Temuan' : 'Laporkan Barang Hilang' }}
    </x-slot>
        ...
        <div class="card">
            <div class="card-header">
                <h3 class="text-lg font-semibold text-gray-900">
                    Form Laporan Barang {{ isset($jenis) && $jenis == 'ditemukan' ? 'Temuan' : 'Hilang' }}
                </h3>
            </div>
            <div class="card-body">
                <form action="{{ route('lost-found.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <label class="relative flex items-center justify-center p-4 border-2 rounded-lg cursor-pointer transition-all
                                {{ (old('jenis', $jenis ?? 'hilang') == 'hilang') ? 'border-red-500 bg-red-50' : 'border-gray-200 hover:border-gray-300' }}">
                                <input type="radio" name="jenis" value="hilang" class="sr-only" {{ (old('jenis', $jenis ?? 'hilang') == 'hilang') ? 'checked' : '' }}>
                                <div class="text-center">
                                    <svg class="w-8 h-8 mx-auto mb-2 {{ (old('jenis', $jenis ?? 'hilang') == 'hilang') ? 'text-red-500' : 'text-gray-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                                    </svg>
                                    <span class="font-medium {{ (old('jenis', $jenis ?? 'hilang') == 'hilang') ? 'text-red-700' : 'text-gray-700' }}">Barang Hilang</span>
                                    <p class="text-xs text-gray-500 mt-1">Saya kehilangan barang</p>
                                </div>
                            </label>
                            <label class="relative flex items-center justify-center p-4 border-2 rounded-lg cursor-pointer transition-all
                                {{ (old('jenis', $jenis ?? 'hilang') == 'ditemukan') ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-gray-300' }}">
                                <input type="radio" name="jenis" value="ditemukan" class="sr-only" {{ (old('jenis', $jenis ?? 'hilang') == 'ditemukan') ? 'checked' : '' }}>
                                <div class="text-center">
                                    <svg class="w-8 h-8 mx-auto mb-2 {{ (old('jenis', $jenis ?? 'hilang') == 'ditemukan') ? 'text-blue-500' : 'text-gray-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
                                    </svg>
                                    <span class="font-medium {{ (old('jenis', $jenis ?? 'hilang') == 'ditemukan') ? 'text-blue-700' : 'text-gray-700' }}">Barang Temuan</span>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="nama_barang" class="form-input @error('nama_barang') border-red-500 @enderror" 
                               value="{{ old('nama_barang') }}" placeholder="Contoh: Dompet Hitam, HP Samsung, dll" required>
                        @error('nama_barang')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="lokasi_ditemukan" class="form-input @error('lokasi_ditemukan') border-red-500 @enderror" 
                               value="{{ old('lokasi_ditemukan') }}" placeholder="Contoh: Kantin Gedung A, Parkiran, dll" required>
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="deskripsi" class="form-textarea @error('deskripsi') border-red-500 @enderror" 
                                  rows="4" placeholder="Jelaskan ciri-ciri barang secara detail..." required>{{ old('deskripsi') }}</textarea>
                        @error('deskripsi')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>

                    @if(isset($lostItems) && $lostItems->isNotEmpty() && (old('jenis', $jenis ?? '') == 'ditemukan'))
                        ...
                        <select name="linked_lost_id" class="form-select">
                            ...
                            @foreach($lostItems as $lostItem)
                                <option value="{{ $lostItem->id }}" {{ old('linked_lost_id', $linked_lost_id ?? '') == $lostItem->id ? 'selected' : '' }}>
                                    {{ $lostItem->nama_barang }} - {{ $lostItem->lokasi_ditemukan }} ({{ $lostItem->user->name }})
                                </option>
                            @endforeach
                        </select>
                        ...
                        <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-lg hover:border-red-400 transition-colors">
                            <div class="space-y-1 text-center">
                                ...
                                <div class="flex text-sm text-gray-600">
                                    <label for="foto" class="relative cursor-pointer rounded-md font-medium text-red-600 hover:text-red-500">
